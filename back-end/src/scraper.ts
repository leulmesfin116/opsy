...
(async () => {
  if (!apiId || !apiHash) {
    console.error(
      "Error: TELEGRAM_API_ID and TELEGRAM_API_HASH must be defined in your .env file.",
    );
    process.exit(1);
  }
  ...
  if (!sessionString) {
    ...
    console.log(
      "Copy this string and save it as TELEGRAM_SESSION in your .env file to avoid logging in every time.\n",
    );
  }
